...
- Does not write migrations, schema SQL, controllers, or servers
- Does not integrate S3, Razorpay, or auth providers
- Does not rewrite frontend pages

---

# 2. Recommended backend stack

## 2.1 Evaluation (do not adopt blindly)

| Layer | Candidate | Verdict | Why |
|---|---|---|---|
| Runtime | **Node.js 22 LTS** | **LOCK** | Frontend is JS; the existing repositories/commands can be ported with the same names. One language for the team. Event-loop is fine for an I/O-bound commerce API. |
| Runtime alt | Go / Java / Python | Reject for V1 | Materially better only at much larger scale or a different hiring profile. Introduces a second language before the first API exists. |
| Process model | **Modular monolith** | **LOCK** | One deployable, one database, in-process transactions for checkout/inventory/payments. Microservices would split the catalogue that Phase 1 just unified. |
| HTTP | **Fastify 5** | **LOCK** | Schema-first validation (JSON Schema), low overhead, first-class async. Matches “server-side schema validation”. |
| HTTP alt | Express | Acceptable fallback | More familiar; weaker built-in validation. Use only if the implementer cannot staff Fastify. |
| HTTP alt | NestJS | Reject for V1 | Extra DI/module ceremony on top of a stack the frontend already models as plain services. Revisit if the team grows. |
| HTTP alt | GraphQL | **Reject** | Frontend already has typed repository methods. GraphQL would invent a second contract and cache invalidation story. REST maps 1:1 onto existing commands. |
| Language | **TypeScript (strict)** | **LOCK** | API contracts, lifecycle enums, and money/inventory types benefit from compile-time checks. Frontend stays JS; the adapter is a thin fetch layer. |
| Language alt | Plain JS | Acceptable | Only if TypeScript onboarding is a blocker. Not recommended. |
| Database | **PostgreSQL 16** | **LOCK** | Relational integrity, `CHECK` constraints, `FOR UPDATE` row locks, JSONB for settings/rules, transactional checkout. The frontend already implies transactions. |
| Database alt | MongoDB | **Reject** | Would re-introduce document snapshots (the exact anti-pattern Phase 1 removed from placements). |
| Query layer | **`pg` + SQL migrations + Kysely** | **LOCK** | Explicit SQL for locks, constraints, and indexes. Kysely gives typed queries without hiding SQL. |
| Query alt | Prisma | Reject as primary | Convenient CRUD; weaker for `SELECT … FOR UPDATE`, partial unique indexes, and CHECK constraints that inventory/payments need. |
| Object storage | **S3-compatible** | **LOCK** | Media bytes do not belong in PostgreSQL or `public/`. AVIF/WebP/JPEG/PNG/MP4/WebM stay as uploaded. |
| CDN | **CloudFront / Cloudflare / R2 public bucket** | **LOCK (later in 3E)** | Immutable object keys + long cache. |
| Auth sessions | **Opaque session tokens in httpOnly cookies** | **LOCK** | Three portals need revocation (suspend, password change). JWT-in-localStorage cannot revoke and is XSS-readable. |
| Auth alt | JWT access + refresh | Acceptable later | Only if cookies are blocked by a future native app. Not needed for the browser SPA. |
| Password hashing | **Argon2id** | **LOCK** | Current fingerprints are not hashes. |
| Validation | Fastify JSON Schema **plus** domain validators in services | **LOCK** | Schema = shape. Domain = lifecycle, money, stock. |
| Payments | **Provider adapter** (Razorpay recommended for India) | **LOCK interface, not vendor** | UPI / cards / netbanking already exist as UI methods. Webhook-only capture. |
| Tests | Node test runner + Supertest + Testcontainers PostgreSQL | **LOCK** | Matches frontend’s Node test runner; real SQL for concurrency tests. |
| Redis / Kafka / ES / K8s | — | **DEFER** | See §44. PostgreSQL handles sessions, rate-limit counters (for V1), search (catalogue size), and jobs. |

## 2.2 Locked V1 stack

```
Frontend:     React 19 + Vite 7 + JavaScript          (unchanged)
Backend:      Node.js 22 + TypeScript (strict) + Fastify 5
API:          REST  /api/v1
DB:           PostgreSQL 16
Migrations:   SQL files (node-pg-migrate or similar)
Queries:      pg pool + Kysely
Files:        S3-compatible object storage
Auth:         httpOnly Secure SameSite cookies, opaque sessions
Hashing:      Argon2id
Payments:     Provider adapter + webhook verification
Jobs:         node-cron / Fastify plugin in the same process (V1)
```

## 2.3 Why not microservices

There is one catalogue, one inventory ledger, one order, one payment confirmation path. Splitting those across networks would recreate the dual-store bugs Phase 1 just removed (`pratikshya_customers` vs registry, assisted orders vs order register). A modular monolith with domain folders is the correct default.

---

# 3. Backend folder architecture

**Do not create these files in Phase 2.** This is the target layout for Phase 3.

```
backend/
├── src/
│   ├── config/            # env, constants, placement catalogue, permission keys
│   ├── middleware/        # auth, RBAC, idempotency, rate-limit, request-id, CSRF
│   ├── routes/            # Fastify route plugins — HTTP only
│   ├── controllers/       # parse request, call service, map HTTP status
│   ├── services/          # business commands (port of productWorkflowCommands etc.)
│   ├── repositories/      # SQL / Kysely — no HTTP, no auth decisions
│   ├── validators/        # JSON Schema + domain validators
│   ├── policies/          # authorization helpers (canPublish, canEditProduct, …)
│   ├── models/            # TypeScript types / row mappers
│   ├── events/            # in-process domain events → audit_logs (NOT Kafka)
│   ├── utils/             # money, ids, time, hashing
│   └── app/               # Fastify factory, plugin registration, health
...
One Node process. One PostgreSQL database. Cross-domain work (checkout) is a **service transaction** that calls multiple repositories inside `BEGIN … COMMIT`.
...
- Customer sign-in today does **not** verify a stored secret. Production **must** — demo logins will break. Seed hashed passwords for demo accounts behind `NODE_ENV !== production`.
...
| `pratikshya_inventory*` | inventory_* | **MIGRATE** empty-unless-stocked (current seed behaviour) |
...
- Helmet headers: CSP (API is not HTML), `nosniff`, `frame-ancestors 'none'`, HSTS
- Rate limit: login 5/min/IP, forgot 3/min, upload 20/min, pay intent 10/min
- Request validation: JSON Schema + domain
...
- SQL: parameterized only (Kysely/pg)
- CSRF: §9.5
...
|---|---|---|
| **3A** Foundation | Fastify app, env, Postgres, migrations runner, error envelope, request-id, CORS, health | — |
...
| TypeScript on backend / JS on frontend | Adapter is untyped JSON; contract tests bind them |
| Fastify vs Express familiarity | Documented fallback |
...
| Component | Why later | Why PostgreSQL/Node cannot yet |
|---|---|---|
...
1. **Stack lock:** Node 22 + TypeScript + Fastify + PostgreSQL 16 + Kysely + S3-compatible storage + cookie sessions.  
...
*End of Phase 2 architecture. No backend code was written.*
