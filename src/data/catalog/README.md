...
| File            | Purpose                                                                 |
| --------------- | ----------------------------------------------------------------------- |
| `products.js`   | One record per product-media folder. The folder id is the permanent product id; `department` / `category` / `subcategory` are read straight from the media path; every product carries a curated, customer-facing name grounded in its own imagery. Prices, descriptions and stock are intentionally absent (`null` / empty) — nothing here invents commercial facts, and incomplete records stay `draft`. |
